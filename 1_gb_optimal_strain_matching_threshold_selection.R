...
load("gb_metadata_subset.RData") # gutbugs metadata with batch info
...
strain_engraftment_analysis <- function(threshold, strain_dna_dist){
  ...
  # find matching strains present in recipient at baseline and week 6
  FMT_BL_wk6_donor_sample_matches <- strain_matches %>% 
    filter(grepl("FMT_wk6|FMT_BL", Sample1_details),    # Sample 1 = recipient samples
    ...
    replace_na(list(BL = "absent", wk6 = "absent")) %>% # replace NAs as strain absent
    mutate(BL = str_replace(BL, "BL", "donor_match"),
           wk6 = str_replace(wk6, "wk6", "donor_match")) %>%
    arrange(Donor_ID, Donor_sample, Recipient_ID, Species) %>%
    group_by(Donor_ID, Recipient_ID, Species) %>%
    filter(any(BL == "donor_match") & any(wk6 == "donor_match")) %>%
    ungroup() %>%
    arrange(Donor_ID, Recipient_ID, Donor_sample, Species)
  
  # baseline-matching strains to subtract from data
  FMT_BL_subtraction <- FMT_BL_wk6_donor_sample_matches %>%
    select(Donor_ID, Recipient_ID, Species) %>%
    distinct()
  
  # subtract matching strains also at baseline
  FMT_engraftment_individual_donors_subBL <- strain_matches %>% 
    filter(grepl("FMT_wk6", Sample1_details),    # Sample 1 = post-FMT recipient sample
           grepl("Donor", Sample2_details)) %>%  # Sample 2 = donor sample
    mutate(Donor_ID = substr(Sample2_details,1,4), # extract donor ID
           Recipient_ID = substr(Sample1_details,1,4)) %>%  # extract recipient ID
    anti_join(FMT_BL_subtraction, by = c("Donor_ID", "Recipient_ID", "Species")) # FMT week 6 donor matches not also in recipient at BL
...
strain_engraftment_analysis_placebo_adjusted <- function(threshold, strain_dna_dist, sea_output){
  ...
  # this function performs strain engraftment analysis (baseline-adjusted) to find donor matches in placebos at week 6
  ...
  # find matching strains present in recipient at baseline and week 6
  placebo_BL_wk6_donor_sample_matches <- strain_matches %>% 
    filter(grepl("Placebo_wk6|Placebo_BL", Sample1_details),    # Sample 1 = recipient samples
           grepl("Donor", Sample2_details)) %>%  # Sample 2 = donor sample
    mutate(Donor_ID = substr(Sample2_details,1,4), # extract donor ID
           Recipient_ID = substr(Sample1_details,1,4), # extract recipient ID
           Timepoint = str_split(Sample1_details, "_", simplify = TRUE)[,3]) %>% # extract timepoint
  ...
  # baseline-matching strains to subtract from data
  placebo_BL_subtraction <- placebo_BL_wk6_donor_sample_matches %>%
    select(Donor_ID, Recipient_ID, Species) %>%
    distinct()
  
  # subtract matching strains also at baseline
  placebo_engraftment_individual_donors_subBL <- strain_matches %>% 
    filter(grepl("Placebo_wk6", Sample1_details),    # Sample 1 = post-placebo recipient sample
           grepl("Donor", Sample2_details)) %>%  # Sample 2 = donor sample
    mutate(Donor_ID = substr(Sample2_details,1,4), # extract donor ID
           Recipient_ID = substr(Sample1_details,1,4)) %>%  # extract recipient ID
    anti_join(placebo_BL_subtraction, by = c("Donor_ID", "Recipient_ID", "Species")) # placebo week 6 donor matches not also in recipient at BL
...
presence_absence_matrix <- function(sea_output, donor_ids, fmtwk6_ids){
  # takes output from strain engraftment analysis and formats as presence absence matrix
  # formatted_sea_output (tibble): presence (1) absence (0) matrix including all individual donors and FMT week 6 recipients
  formatted_sea_output <- sea_output %>%
    ...
    mutate(value = ifelse(is.na(value), 0, value), # mark any missing donor IDs as absent
           Recipient_ID = ifelse(is.na(Recipient_ID), "TF01", Recipient_ID)) %>% # replace missing paired recipient ID with arbitrary ID
    
    right_join(fmtwk6_ids, by = c("Recipient_ID" = "Participant_ID")) %>% # join with all FMT wk6 IDs to make sure none missing
    mutate(value = ifelse(is.na(value), 0, value), # mark any missing donor IDs as absent
           Donor_ID = ifelse(is.na(Donor_ID), "DF12", Donor_ID)) %>% # replace missing paired donor ID with arbitrary ID
...
sea_f1_multiple_thresholds <- function(strain_dna_dist, threshold_min, threshold_max, threshold_interval, donor_ids, fmtwk6ids, true_vector){
  ...
  # fmtwk6ids: all FMT recipient week 6 IDs
  ...
  for (threshold in thresholds) {
    ...
    formatted_sea_output <- presence_absence_matrix(sea_output, donor_ids, fmtwk6_ids)
...
sea_f1_multiple_thresholds_placebo_subtraction <- function(strain_dna_dist, threshold_min, threshold_max, threshold_interval, donor_ids, fmtwk6ids, true_vector){
  # strain_dna_dist: DNA distances from pairwise strian comparisons (from strainphlan output)
  # threshold_min: minimum DNA dist threshold to test (must be > 0)
  # threshold_max: maximum DNA dist threshold to test
  # threshold_interval: DNA dist threshold intervals to test
  ...
  for (threshold in thresholds) {
    ...
    # format strain engraftment analysis results as presence absence matrix including all donor and recipient IDs
    formatted_sea_output_placebo_adjusted <- presence_absence_matrix(sea_output_placebo_adjusted, donor_ids, fmtwk6_ids)
...
# density plot to decide the thresholds to use as a "strain-match"
density_plot_stringent <- strain_dna_dist %>% 
  ...
  filter(Dist_norm <= 3) %>% # remove large tail 
...
# filter Gut Bugs metadata for individual donor IDs (n = 9) and FMT week 6 IDs (n = 39)
# used later to make sure predicted data includes all individual donors and FMT recipients at week 6
donor_ids <- gb_metadata %>%
  filter(Group == "Donor") %>% # individual donors
  select(Participant_ID) %>% # select participant ID
  distinct() # get distinct rows (multiple donor samples)

fmtwk6_ids <- gb_metadata %>%
  filter(Group == "FMT" & Timepoint == "wk6") %>% # FMT recipients
  select(Participant_ID) %>% # select participant ID
  distinct() # get distinct rows (multiple donor samples)
...
formatted_true_pairings <- true_pairings_formatted %>%
  ...
  right_join(fmtwk6_ids, by = c("Recipient_ID" = "Participant_ID")) %>% # join with all FMT wk8 IDs to make sure none missing
  mutate(n = ifelse(is.na(n), 0, n), # mark any missing donor IDs as absent
         Donor_ID = ifelse(is.na(Donor_ID), "DF12", Donor_ID)) %>% # replace missing paired donor ID with arbitrary ID
...
sea_f1_multiple_thresholds_res <- sea_f1_multiple_thresholds(strain_dna_dist, threshold_min = 0.001, threshold_max = 3, threshold_interval = 0.001,
                                                             donor_ids, fmtwk6ids, true_vector)
...
sea_f1_multiple_thresholds_both_methods[1:10,] # 0.123 (with no placebo adjustment) where F1 = 0.791

# run strain engraftment analysis with optimal threshold to predict FMT donor-recipient pairings
sea_output_best <- strain_engraftment_analysis(0.123, strain_dna_dist)

# format strain engraftment analysis results as presence absence matrix including all donor and recipient IDs
formatted_sea_output_best <- presence_absence_matrix(sea_output_best, donor_ids, fmtwk6_ids)
...
# compare with confusion matrix for previously used threshold (0.2), which didn't use placebo adjustment in strain engraftment analysis
sea_output_past <- strain_engraftment_analysis(0.2, strain_dna_dist)
formatted_sea_output_past <- presence_absence_matrix(sea_output_past, donor_ids, fmtwk6_ids)
predicted_vector_past <- as.vector(as.matrix(formatted_sea_output_past))
confusion_matrix_past <- confusionMatrix(factor(predicted_vector_past), factor(true_vector), positive="1")
...
max(strain_dna_dist$Dist_norm) # 122529.9
# for DNA dist thresholds from 4-122530, test these at intervals of 1000

# extra results for baseline-adjusted strain engraftment analysis
sea_threshold_testing_extra_res <- sea_f1_multiple_thresholds(strain_dna_dist, threshold_min = 4, threshold_max = 122530, threshold_interval = 1000,
                                                              donor_ids, fmtwk6ids, true_vector)
...
sea_threshold_testing_extra_res_placebo_adjusted <- sea_f1_multiple_thresholds_placebo_subtraction(strain_dna_dist, 
                                                                                                   threshold_min = 4, threshold_max = 122530, threshold_interval = 1000,
                                                                                                   donor_ids, fmtwk6ids, true_vector)
